...
public class Recursive {
   ...
   // Helper method?? We don't need no stinkin' helper method!!
    ...
    private void drawTriangles(int minSideLength, double currentSideLength,  double x1, double y1, Graphics g) {
    	
    	double x2 = x1 + 0.25 * currentSideLength;
		double y2 = y1 - Math.sqrt( 3 ) * currentSideLength / 4;
    ...
    private static void drawSquares(Graphics g, int size, int limit, double x, double y) {
    	...
    	if( size > limit ) {
    			...
    			g.setColor( Color.WHITE );
    			g.fillRect( (int) ( x + x / 3 ), (int) ( y + y / 3 ), size / 3, size / 3 );
    	}
